package com.pisces.framework.rds.helper.resolve;
...
import com.pisces.framework.rds.annotation.KeySql;
import com.pisces.framework.rds.annotation.NameStyle;
import com.pisces.framework.rds.annotation.Order;
import com.pisces.framework.rds.entity.*;
import com.pisces.framework.rds.helper.FieldHelper;
import com.pisces.framework.rds.utils.GenSql;
...
public class DefaultEntityResolve implements EntityResolve {
    ...
    public EntityTable resolveEntity(Class<?> entityClass, Config config) {
        ...
        if (entityTable == null) {
            entityTable = new EntityTable(entityClass);
            //可以通过stye控制
            String tableName = StringUtils.convertByStyle(entityClass.getSimpleName(), style);
            //自动处理关键字
    ...
    protected void processGeneratedValue(EntityTable entityTable, EntityColumn entityColumn, GeneratedValue generatedValue) {
        if ("JDBC".equals(generatedValue.generator())) {
            ...
            if (generatedValue.strategy() == GenerationType.IDENTITY) {
                ...
                throw new SystemException(entityColumn.getProperty()
    ...
    protected void processKeySql(EntityTable entityTable, EntityColumn entityColumn, KeySql keySql) {
        ...
        } else if (StringUtils.isNotEmpty(keySql.sql())) {
