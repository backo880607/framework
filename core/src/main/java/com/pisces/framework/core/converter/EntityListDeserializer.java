 ...
 * @date 2022/12/07
 */
public class EntityListDeserializer extends BaseDeserializer<Collection<BaseObject>> {
    private static final String ENTITY_SPLIT = ";";

    @SneakyThrows
    @Override
    public Collection<BaseObject> deserialize(JsonParser p, DeserializationContext context) throws IOException {
        ...
        for (JsonNode node : nodeList) {
            BaseObject entity = (BaseObject) property.getTypeClass().newInstance();
    ...
    @Override
    public Collection<BaseObject> deserialize(Property property, String value) {
        ArrayList<BaseObject> entities = new ArrayList<>();
