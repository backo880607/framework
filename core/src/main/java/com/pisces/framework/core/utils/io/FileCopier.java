...
public class FileCopier extends SrcToDestCopier<File, FileCopier> {
    ...
    public File copy() {
        ...
        // check
        if (!src.exists()) {
            throw new SystemException("File not exist: " + src);
        }
        if (FileUtils.equals(src, dest)) {
            throw new SystemException(MessageFormat.format("Files '{}' and '{}' are equal", src, dest));
        }
        ...
        if (src.isDirectory()) {
            ...
            if (dest.exists() && !dest.isDirectory()) {
                //源为目录，目标为文件，抛出IO异常
                throw new SystemException("Src is a directory but dest is a file!");
            }
            if (FileUtils.isSub(src.toPath(), dest.toPath())) {
                throw new SystemException("Dest is a sub directory of src !");
            }

            final File subTarget = isCopyContentIfDir ? dest : FileUtils.mkdir(new File(dest, src.getName()));
    ...
    private void internalCopyDirContent(File src, File dest) {
        ...
        if (!dest.exists()) {
            ...
            throw new SystemException(MessageFormat.format("Src [{}] is a directory but dest [{}] is a file!", src.getPath(), dest.getPath()));
        }
