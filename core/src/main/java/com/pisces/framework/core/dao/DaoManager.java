...
public class DaoManager {
    ...
    private static final List<BaseDao<? extends BaseObject>> DAO_LIST = new ArrayList<>();
    private static final List<BaseDao<? extends BaseObject>> MEMORY_DAO = new ArrayList<>();

    protected DaoManager() {
    }

    public static void register(BaseDao<? extends BaseObject> dao) {
        DAO_LIST.add(dao);
    }

    public static void registerMemory(BaseDao<? extends BaseObject> dao) {
        register(dao);
        MEMORY_DAO.add(dao);
    }

    public static void sync() {
        for (BaseDao<? extends BaseObject> dao : DAO_LIST) {
            dao.sync();
        }
    }
        ...
        if (!userData.bInit) {
            synchronized (DaoManager.class) {
                if (!userData.bInit) {
                    ...
                    for (Map.Entry<BaseDao<? extends BaseObject>, DaoImpl> entry : userData.daoImpl.entrySet()) {
    ...
    private static class UserData {
        boolean bInit;
        Map<BaseDao<? extends BaseObject>, DaoImpl> daoImpl = new HashMap<>();
    }
}
